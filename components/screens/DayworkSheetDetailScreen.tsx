...
import { Project, DayworkSheet, User } from '../../types.ts';
...
const DayworkSheetDetailScreen: React.FC<DayworkSheetDetailScreenProps> = ({ sheetId, project, goBack, currentUser }) => {
    ...
    useEffect(() => {
        const loadSheet = async () => {
            setIsLoading(true);
            const fetchedSheet = await api.fetchDayworkSheetById(sheetId);
    ...
    const canApprove = (currentUser.role === 'company_admin' || currentUser.role === 'supervisor') && sheet.status === 'Pending';
                            ...
                            <div>
                             ...
                             {sheet.approvedBy && (
                                <div>
                                    ...
                                    <p className="text-gray-800 font-semibold">{sheet.approvedBy}</p>
