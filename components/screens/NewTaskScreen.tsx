...
import { Project, User, UserRole } from '../../types.ts';
// Fix: Added .ts extension to import
import * as api from '../../api.ts';
// Fix: Added .tsx extension to import
// Change: Swapped XMarkIcon for TrashIcon for photo removal UI.
import { ChevronLeftIcon, CalendarDaysIcon, UsersIcon, CameraIcon, TrashIcon, SparklesIcon, ArrowPathIcon } from '../Icons.tsx';
...
const ROLES_AVAILABLE_FOR_ASSIGNMENT: UserRole[] = ['operative', 'supervisor'];
...
const NewTaskScreen: React.FC<NewTaskScreenProps> = ({ project, goBack, currentUser }) => {
    ...
    const sortedAssignees = useMemo(() => {
        const suggestedIds = aiSuggestion?.suggestedAssigneeIds || [];
        if (suggestedIds.length === 0) {
            return allUsers;
        }
        const suggested = suggestedIds.map(id => allUsers.find(u => u.id === id)).filter(Boolean) as User[];
        const rest = allUsers.filter(u => !suggestedIds.includes(u.id));
                                ...
                                <optgroup label="Users">
                                    {sortedAssignees.map(user => (
                                        <option key={user.id} value={`user:${user.id}`}>
                                            {aiSuggestion?.suggestedAssigneeIds?.includes(user.id) ? '✨ ' : ''}
                                            {user.name} ({user.role})
                                        </option>
                                    ))}
                                </optgroup>
